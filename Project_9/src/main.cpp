...
void publishSensorReadings() {
  ...
  mqttClient.publish(TOPIC_SENSOR_TEMPERATURE, buf, true);

  dtostrf(bme280.getHumidity(), 0, 2, buf);
  mqttClient.publish(TOPIC_SENSOR_HUMIDITY, buf, true);

  dtostrf(bme280.getPressure(), 0, 2, buf);
  mqttClient.publish(TOPIC_SENSOR_PRESSURE, buf, true);
}

void publishMotorStatus() {
  mqttClient.publish(TOPIC_MOTOR_STATUS_STATE, motor.isRunning() ? "1" : "0", true);
  mqttClient.publish(TOPIC_MOTOR_STATUS_DIRECTION, motor.isForward() ? "forward" : "reverse", true);

  char buf[8];
  itoa(motor.getSpeedPercent(), buf, 10);
  mqttClient.publish(TOPIC_MOTOR_STATUS_SPEED, buf, true);
}
