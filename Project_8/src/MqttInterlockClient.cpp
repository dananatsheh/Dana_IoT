...
void MqttInterlockClient::publish(const char* topic, const char* payload, bool retained) {
  if (!_mqttClient.connected()) {
    return;
  }
...
void MqttInterlockClient::connectBroker() {
  ...
  if (strlen(MQTT_USERNAME) > 0) {
    connected = _mqttClient.connect(
      ...
      TOPIC_DEVICE_STATUS, DEVICE_STATUS_QOS, true, DEVICE_STATUS_OFFLINE);
  } else {
    connected = _mqttClient.connect(
      clientId.c_str(),
      TOPIC_DEVICE_STATUS, DEVICE_STATUS_QOS, true, DEVICE_STATUS_OFFLINE);
  ...
  if (connected) {
    ...
    _mqttClient.publish(TOPIC_DEVICE_STATUS, DEVICE_STATUS_ONLINE, true);
    resubscribeAll();
    _reconnectFlag = true;
  } else {
